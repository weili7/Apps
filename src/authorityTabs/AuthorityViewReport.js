...
import {moveFirebase, moveAuthFirebase} from '../components/RealTimeDatabase'
...
class ViewSubmittedReport extends React.Component{
	...
	constructor(props) {
		...
		this.state = {
			...
			*/
			...
			addCategory:0,
			reporterid:this.props.navigation.state.params.userid,
   ...
   //Alert.alert("hello", "more")
	...
	onSetRes=async()=>{
		this.refs.addRespond.close()
		this.setState({res:this.state.resp})
	...
	_waiting=async()=>{
		this.refs.respond.close()
		const ready = await editReportDetails("Reports",this.state.reportid,'progress',0).catch((error) => {console.log(error)})
		this.setState({authProgress:0})
	}


	_underProgress=async()=>{
		this.refs.respond.close()
		const ready = await editReportDetails("Reports",this.state.reportid,'progress',1).catch((error) => {console.log(error)})
		this.setState({authProgress:1})
	}


	_complete=async()=>{
		Alert.alert("Complete The Progress", "Please confirm your action, The action cannot be undo!!",[{text: 'No'},{text: 'Yes', onPress: () => this._completeReport()}]);
	}

	_completeReport=async()=>{
		this.refs.respond.close()
		const ready = await editReportDetails("Reports",this.state.reportid,'progress',2).catch((error) => {console.log(error)})
		const ready2 = await moveFirebase(this.state.reporterid, this.state.reportid)
		const ready3 = await moveAuthFirebase(this.state.authorityid, this.state.reportid)
		this.setState({authProgress:2})
	}
	...
	render(){
		...
		return(
			...
			<ScrollView contentContainerStyle={styles.container}>
				...
				<View style={styles.R_box}>
					...
					<TouchableOpacity
						...
						<Text style={{textAlign:'right', color:'rgba(255,119,26,0.85)',marginLeft:width-220, marginTop:2, fontSize:11}}> ADD </Text>
					</TouchableOpacity>
				</View>
				<View style={{
					flex:1,
				...
				<View style={{
					...
					<TouchableOpacity
						...
						<Icon
							...
							type='entypo'/>
						<Text style={{marginTop: 10, marginLeft:10,}}>{this.state.respond[this.state.authProgress]}</Text>
					</TouchableOpacity>
				</View>
			...
			<Modal style={{
				...
				borderRadius: 20,
				shadowRadius:10,}}
				position={"center"} backButtonClose={true} ref={"addRespond"} isDisabled={this.state.isDisabled}>
				<Text style={{marginTop:20, fontSize: 20, alignItems:"center"}}>Respond</Text>
				<TextInput
					style={{width:width-70, marginTop:20, marginBottom:20}}
					multiline={true}
					numberOfLines = {3}
					onChangeText={resp => this.setState({resp})}/>
				<Button
					title="OK"
					buttonStyle={{ backgroundColor:'rgba(255,119,26,0.85)', marginTop: 5 , width: width/3, borderRadius:15,}}
						onPress={this.onSetRes.bind(this)}
				/>
			</Modal>
			...
			<Modal style={{
				...
				<Text style={{marginTop:20, fontSize: 20}}>Status</Text>
				<ListItem
					containerStyle={{marginTop:25, width:width-80, marginLeft:20, marginRight:20, height:40, borderBottomColor:'lightgrey',  borderTopWidth:1, borderTopColor:'lightgrey',}}
					title={this.state.respond[0]}
					titleStyle={{color:'grey', marginLeft:20}}
					onPress={()=>{this._waiting()}}
					leftIcon={{color: 'rgba(255,119,26,0.85)', name:this.state.progress[0], size: 18, type:'entypo'}}
				/>
				<ListItem
					containerStyle={{marginTop:25, width:width-80, marginLeft:20, marginRight:20, height:40, borderBottomColor:'lightgrey',  borderTopWidth:1, borderTopColor:'lightgrey',}}
					title={this.state.respond[1]}
					titleStyle={{color:'grey', marginLeft:20}}
					onPress={()=>{this._underProgress()}}
					leftIcon={{color: 'rgba(255,119,26,0.85)', name:this.state.progress[1], size: 18, type:'entypo'}}
				/>
				<ListItem
					containerStyle={{marginTop:25, width:width-80, marginLeft:20, marginRight:20, height:40, borderBottomColor:'lightgrey',  borderTopWidth:1, borderTopColor:'lightgrey',}}
					title={this.state.respond[2]}
					titleStyle={{color:'grey', marginLeft:20}}
					onPress={()=>{this._complete()}}
					leftIcon={{color: 'rgba(255,119,26,0.85)', name:this.state.progress[2], size: 18, type:'entypo'}}
				/>
			</Modal>
			</View>
